...
def build_index(chunks):
   ...
   #บันทึก index ลงไฟล์ถาวร
   #faiss.write_index(rag.index, "artifacts/index.faiss")
   return index
...
//ให้ปรับ rag_utils.py ให้สร้างเป็นชุดเล็กๆ
batch_size = 20
for i in range(0, len(texts), batch_size):
    batch = texts[i:i+batch_size]
    embeddings = llm.embed(batch)

//ถ้า มันตอบไม่รู้เรื่อง ให้
เพิ่ม SIM_THRESHOLD, ลด temperature
